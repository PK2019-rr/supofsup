...
SYSTEM_PROMPT = (
    ...
    "Если в ответе присутствуют ссылки — выводи их в формате Markdown: [текст](url)."
)
...
def build_user_prompt(user_query, search_summary=None):
    ...
    if search_summary:
        ...
        prompt += "Ответь, используя эти источники. Добавь не менее трёх ссылок в формате Markdown-гиперссылок, если это возможно."
    else:
        prompt += "Ответь максимально точно, используя твои знания. Если можешь — приведи три ссылки в формате Markdown."
    return prompt
